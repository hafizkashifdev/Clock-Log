...
const AddModal = (props: IProps) => {
  ...
  return (
    <>
      <Modal title={`${isAddModal.id} Work Schedule`} open={isAddModal.isOpen} footer={false} onCancel={() => setIsAddModal(false)} className="add-work-modal-wrapper">
        <div className="add-work-wrapper">
          <Form layout="vertical" onFinish={onFinish} form={form}>
            <Row gutter={[22, 16]}>
              <Col span={24} className="apply-work-field-wrap">
                <Form.Item label="Shift Name" name="shiftName" rules={[{ required: true, message: "required input" }]}>
                  <Input placeholder="Basic usage" style={{ height: "37px" }} className="container-400" />
                </Form.Item>
              </Col>
              <Col xs={24} sm={24} md={24} lg={8} className="apply-work-field-wrap">
                <Form.Item label="Shift Start Time" name="shftStartTime" rules={[{ required: true, message: "required input" }]}>
                  <TimePicker format="h:mm A" value={shiftStartTime} onChange={handleStartTimeChange} style={{ width: "100%", maxWidth: "auto", height: "37px" }} />
                </Form.Item>
              </Col>
              <Col xs={24} sm={24} md={24} lg={8} className="apply-work-field-wrap">
                <Form.Item label="Shift End Time" name="shftEndTime" rules={[{ required: true, message: "required input" }]}>
                  <TimePicker format="h:mm A" value={shiftEndTime} onChange={handleEndTimeChange} style={{ width: "100%", maxWidth: "auto", height: "37px" }} />
                </Form.Item>
              </Col>
              <Col xs={24} sm={24} md={24} lg={8} className="apply-work-field-wrap">
                <p className="fs-14 fw-600 tag-overall-time">
                  Total Hours: <span className="fs-20 fw-500 secondary-color">{calculateTotalShiftHours() ? calculateTotalShiftHours()?.toFixed(1) : "0"}h</span>
                </p>
              </Col>
              <Col xs={24} sm={24} md={24} lg={8} className="apply-work-field-wrap">
                <Form.Item label="Break Start Time" name="brkStarttime" rules={[{ required: true, message: "required input" }]}>
                  <TimePicker format="h:mm A" value={breakStartTime} onChange={handleBreakStartTimeChange} style={{ width: "100%", maxWidth: "auto", height: "37px" }} />
                </Form.Item>
              </Col>
              <Col xs={24} sm={24} md={24} lg={8} className="apply-work-field-wrap">
                <Form.Item label="Break End Time" name="breakEndTime" rules={[{ required: true, message: "required input" }]}>
                  <TimePicker format="h:mm A" value={breakEndTime} onChange={handleBreakEndTimeChange} style={{ width: "100%", maxWidth: "auto", height: "37px" }} />
                </Form.Item>
              </Col>
              <Col xs={24} sm={24} md={24} lg={8} className="apply-work-field-wrap">
                <p className="fs-14 fw-600 tag-overall-time">
                  Min.Effective Hours: <span className="fs-20 fw-500 secondary-color">{calculateTotalBreakHours() ? calculateTotalBreakHours()?.toFixed(1) : "0"}h</span>
                </p>
              </Col>
              <Col xs={24} sm={24} md={24} lg={8} className="apply-work-field-wrap">
              ...
              <Col md={24} style={{ display: "flex", justifyContent: "end", gap: "8px" }}>
                <Button className="btn-cancel" onClick={() => setIsAddModal({ isOpen: false, id: "" })}>
                  Cancel
                </Button>
      ...
      <Modal
        ...
        className="relaxation-time-modal-wrapper"
      >
