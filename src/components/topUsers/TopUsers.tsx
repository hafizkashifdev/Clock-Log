...
import { ArrowLeftOutlined, SearchOutlined, InfoCircleOutlined } from "@ant-design/icons";
...
const TopUsers = (props: any) => {
  ...
  const UserTableConstants = () => {
    ...
    IsShowTableColumns.includes("Overall Score") && (
      columns.push({
        title: <div className="column-data">Overall Score<Tooltip overlayInnerStyle={{ width: '100%', minWidth: '360px' }} placement="top" title={TooltipsData.overall} color="#FCFCFD"><InfoCircleOutlined style={{ marginLeft: '8px', fontSize:'17px' }} /></Tooltip></div>,
    ...
    IsShowTableColumns.includes("Productivity Score") && (
      columns.push({
        title: <div className="column-data">Productivity Score<Tooltip overlayInnerStyle={{ width: '100%', minWidth: '360px' }} placement="top" title={TooltipsData.productivityScore} color="#FCFCFD"><InfoCircleOutlined style={{ marginLeft: '8px', fontSize:'17px' }} /></Tooltip></div>,
    ...
    IsShowTableColumns.includes("Idle Time Score") && (
      columns.push(
        {
          title: <div className="column-data">Idle Time Score<Tooltip overlayInnerStyle={{ width: '100%', minWidth: '360px' }} placement="top" title={TooltipsData.idleTimeScore} color="#FCFCFD"><InfoCircleOutlined style={{ marginLeft: '8px', fontSize:'17px' }} /></Tooltip></div>,
    ...
    IsShowTableColumns.includes("Time Tracked Score") && (
      columns.push(
        {
          title: <div className="column-data">Time Tracked Score<Tooltip overlayInnerStyle={{ width: '100%', minWidth: '360px' }} placement="top" title={TooltipsData.timeTrackedScore} color="#FCFCFD"><InfoCircleOutlined style={{ marginLeft: '8px', fontSize:'17px' }} /></Tooltip></div>,
      ...
      <div className="top-users-wrapper">
        <div className="topUserBar">
          ...
          <div className="rightSearch" style={{ alignItems: "center", display: "flex", justifyContent: "space-between", gap: "10px" }}>
            ...
            {searchFilter &&
              <Input style={{ height: "40px" }} placeholder="Search by user name" className="topUsers-search" onChange={(e) => console.log(e.target.value)} prefix={<SearchOutlined className="site-form-item-icon" />} />
            }
            <div style={{ marginLeft: '10px' }}>
              <ColumnDropdownWrapper menuValue={IsShowTableColumns} setMenuValue={setIsShowTableColumns} options={dropDownOptions} >
                <Button className='d-flex align-center column_btn'>
