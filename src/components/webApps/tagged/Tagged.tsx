...
const options = [
    ...
    const columns: ColumnsType<DataType> = [
        ...
        },
        {
            ...
            render: () => (
                <div  className={`ProductivityStatusTagged ${selectedOption==='Neutral'?'NeutralStatus':selectedOption==='Productive'?'ProductiveStatus':selectedOption==='Unproductive'?'UnproductivelStatus':"ProductivityStatusTagged"}`}>
                <Select 
                // defaultValue='Productivity Status'
                // className="select-input-theme"
                // popupClassName="select-theme"
                ...
                >
                    {options.map(option => (
                        <Select.Option key={option.value} value={option.value} 
                         style={{backgroundColor:option.value==='Neutral'?'#EBEAEA':option.value==='Productive'?'#D9FFDA':option.value==='Unproductive'?'#FFCBCB':"",borderRadius:'25px',width:'fit-content',marginTop:"10px",color:option.value==='Neutral'?'#4E4B66':option.value==='Productive'?'#0FBE16':option.value==='Unproductive'?'#FF3737':""}}>
                ...
                <div>
                    <Row className="table_header d-flex justify-between">
                        ...
                        <Col xs={24} md={8} lg={4} className="search_wrapper">
                            <Input prefix={<SearchOutlined />} placeholder="Serach by Distraction Name" />
                        </Col>
                    </Row>
                </div>
            </div>
