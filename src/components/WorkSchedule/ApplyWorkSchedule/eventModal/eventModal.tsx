...
const EventModal = (props: any) => {
    const { isEventModal, setIsEventModal, eventModalData } = props;
    ...
    return (
        <>
            <Modal title="Shift" open={isEventModal} onOk={() => setIsEventModal(false)} onCancel={() => setIsEventModal(false)} className="work-schedule-modal" okText="Edit" footer={[
                <div className='event-modal-btn d-flex'>
                    ...
                    <button className='button-modal-edit grey-color fs-16 fw-500 line-height-24 cursor-pointer' type='button'>Edit</button>
                </div>
            ]}>
                <div className='event-modal-details'>
                    <div className='d-flex align-center' style={{ gap: "10px" }}>
                        ...
                        <div className='grid-circle'>
                            ...
                            <p className='fs-12 fw-500 m-0 d-flex align-center'><span style={{ backgroundColor: event?.ui?.borderColor }}></span>{event?.extendedProps?.shift}</p>
                        </div>
                    </div>
