        ...
        <Card className="overallscore overallscroe-card border-radius-8 card-bg-color" bordered={false} style={{ boxShadow: "0px 1px 6px rgba(0, 0, 0, 0.15)" }}>
          <div className="attendance-summary d-flex justify-between">
            <div style={{ display: "flex" }}>
              ...
              <Tooltip overlayStyle={{ width: '310px', height: "58px", borderRadius: "4px" }} placement="right" title={<span className='fw-400 fs-14' style={{ color: "#FCFCFD" }}>Users with top overall score from the top
                users report</span>}>
            ...
            <div className="filter-score">
              <Row>
                <Col sm={12} style={{ borderRight: "1px solid #a0acbb" }}>
                  <span className="showmore  lowest fw-500 fs-16 " onClick={() => { setSelectedProductivityLevel(1); setHighestLinkColor("#E76F51"); setLowestLinkColor(""); }} style={{ color: highestLinkColor }}>Highest</span>

                </Col>
                <Col sm={12} className="lowestfiltertext">
                  <span className="lowest fs-16" onClick={() => { setSelectedProductivityLevel(2); setHighestLinkColor(""); setLowestLinkColor("#E76F51"); }} style={{ color: lowestLinkColor, margin: "10px" }}>Lowest</span>
                </Col>
              </Row>
          ...
          {selectedProductivityLevel === 2 && OverAllSCore.slice(0, 5)?.map((score) => {
            return (
              <Row gutter={16} key={uuidv4()}>
                ...
                <Col lg={5} md={5} xs={24} sm={24} style={{ padding: "0px" }}>
                  <div className="name-designation" style={{ marginLeft: "5px" }}>
                    <p className="name m-0  fw-500 fs-14 black-color">{score.lowestFilterNameValue}</p>
                    <p className="designation fs-10 fw-400 light-grey-color-2">{score.designation}</p>
                  </div></Col>
                <Col lg={6} md={6} xs={24} sm={24} className="score-value dark-grey-color  text-center">
                  <span className='fs-14 dark-grey-color'>{score.score}/100</span>
                </Col>
                <Col lg={10} md={10} xs={24} sm={24} style={{ paddingRight: "0px", marginTop: "12" }} >
                  <DemoProgress score={score.score} />
                </Col>
              </Row>
            );
