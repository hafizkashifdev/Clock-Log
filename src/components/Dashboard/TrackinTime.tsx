        ...
        <Card className="tracking-time-card border-radius-8 card-bg-color" bordered={false} style={{ minHeight: "200px", boxShadow: "0px 1px 6px rgba(0, 0, 0, 0.15)", paddingBottom: "6px" }}>
          <div className="attendance-summary d-flex justify-between" >
            <div style={{ display: "flex" }}>
              ...
              <Tooltip overlayStyle={{ width: '316px', height: "79px", borderRadius: "4px" }} placement="right" title={<span className='fw-400 fs-14' style={{ color: "#FCFCFD" }}>A list of user who either haven’t accepted
            ...
            <Link to="/reports/hour-tracked" className='showmore' style={{ color: "#A0ACBB" }}>Manage Users</Link>
          </div>
            ...
            return (
              <Row key={uuidv4()} gutter={16} style={{ paddingBottom: "5px" }}>
                ...
                <Col xl={10} lg={24} md={24} sm={24} xs={24} style={{ paddingLeft: "0px", marginTop: "10px" }}>
                  ...
                  <p className='m-0 fw-300 fs-10 dark-grey-color tracking-time-lastSeen-text' style={{ textIndent: '8.5em' }}>{data.lastSeen}</p>
                </Col>
                <Col xl={10} lg={24} md={24} sm={24} xs={24} className="tracking-time-buttons text-center " style={{ paddingLeft: "0px", marginTop: "15px" }}>

                  <Button className={data.buttonText === "Resend" ? "resend-button" : ""} style={{ display: "inline-block", }} danger size="small" ><span className='d-flex align-center'> <img src={data.imageHide} style={{ color: "red" }} width={14} height={14} className="resendButton text-center" /><span >{data.buttonText}</span></span></Button>
                </Col>
              </Row>
            )
          })}
