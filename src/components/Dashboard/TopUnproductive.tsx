...
export const TopUnproductive = ({selectedOptions}:any) => {
  ...
  return (
    <Layout className="dashboard">
       {selectedOptions.includes("Unproductive Services") &&  <Card className="overallscore border-radius-8" style={{ boxShadow: "0px 1px 6px rgba(0, 0, 0, 0.15)" }}>
        <div className='attendance-summary d-flex justify-between' style={{marginBottom:"20px"}}>
          <div style={{ display: "flex" }}>
            <p className="fw-600 m-0 fs-16">Top Unproductive Services</p>
          </div>
            ...
            <Row gutter={16} key={uuidv4()} style={{paddingBottom:"21px"}}>
              <Col xl={8} lg={12} md={12} xs={12} sm={12}>
                <div className="d-flex align-center" style={{marginTop:"3px"}}>
                  <img src={Unproductive} alt="attendance" className="icon" width={24} height={24} />
                  <span className=" unproductive-sites fs-14 fw-500 " style={{color:"#3B4D60",paddingLeft:"6px"}}>{score.unProductive}</span>
                </div>
              </Col>
              <Col xl={8} lg={12} md={12} xs={12} sm={12} style={{textAlign:"center",marginTop:"2px"}} >
                  <span className=" fs-14 fw-500">{score.manualTime}</span>
              </Col>
...
export const RecentUnratedServices = ({selectedOptions}:any) => {
  return (
    <Layout className="dashboard">
      ...
      <Card className="overallscore border-radius-8" style={{ boxShadow: "0px 1px 6px rgba(0, 0, 0, 0.15)" }}>
        <div className='attendance-summary d-flex justify-between ' style={{marginBottom:"20px"}}>
          
            <p className="fw-600 m-0 fs-16">Recent Unrated Services</p>
...
export const EmploeeMostUsedServices = ({selectedOptions}:any) => {
  return (
    <Layout className='dashboard bg-unset'  >
      {selectedOptions.includes("Employee Most Used Services")  && <Card className="most-used-services-card-employee  border-radius-8 card card-bg-color" bordered={false} style={{minHeight:"428px",boxShadow: "0px 1px 6px rgba(0, 0, 0, 0.15)",paddingBottom:"6px"}}>
