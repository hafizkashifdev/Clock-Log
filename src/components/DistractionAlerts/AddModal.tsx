...
function AddModal(props: any) {
  ...
  console.log(addDistraction.id === "Add")
  const onFinish = () => {
  ...
  return (
      ...
      title={
        !selectedUsers ? (
          ...
          <span className="fs-24 d-flex align-items-center">
            ...
            <span className="title-color fs-24 fw-600">Select Users</span>          </span>)
      }
    ...
    >      {!selectedUsers && (
      <Form layout="vertical" onFinish={onFinish} form={form}>
        <Row gutter={[16, 8]}>
          <Col xs={24} md={12} style={{ paddingTop: "8px" }}>
            <Form.Item label="Distraction Name" className="input-distraction-alerts" name={["distractionName"]} rules={[{ required: true, message: "Input required" }]}>                <Input placeholder="Name" style={{ borderRadius: "2px", height: "37px" }} />              </Form.Item>            </Col>          </Row>          <Row>            <Col xs={24} md={12}>              <h4 className="m-0 p-0 title-color fs-20 fw-600" style={{ paddingBottom: "10px" }}>Conditions</h4>              <Form.Item label="Parameter" name={["condition"]} rules={[{ required: true, message: "required field*" }]}  >                <Select
            ...
            <Select
             ...
             <Col xs={24} md={12} className="operator-input">              
             <Form.Item label="Time" className="input-distraction-alerts" name="time" rules={[{ required: true, message: "Input required" }]} >                <Input type="Number" placeholder="08:10 AM" style={{ borderRadius: "2px", height: "37px" }} />              </Form.Item>            </Col>            <Col xs={24} md={12}>              <Form.Item label="Triggers in a Week" className="input-distraction-alerts" name="triggers" rules={[{ required: true, message: "Input required" }]} style={{ paddingTop: "6px" }}>                <Input type="Number" placeholder="3" style={{ borderRadius: "2px", height: "37px" }} />              </Form.Item>            </Col>            <Col xs={24} md={24}>              <h4 className="m-0 p-0 title-color fs-20 fw-600">Email Body</h4>              <Form.Item name="textArea" rules={[{ required: true, message: "Input required" }]}>                <Input.TextArea style={{ paddingTop: "10px" }} className="my-textarea-class" showCount maxLength={100} placeholder="The Orcalo Ltd Distraction Alert report is attached. Please download, Thankyou." />              </Form.Item>              <h4 className="p-0 m-0 title-color fs-20 fw-600" style={{ paddingBottom: "10px" }}>Select Users</h4>              <Form.Item name="selectUser" >                <Radio.Group onChange={onRadioChange} value={radioValue} style={{ display: "flex", flexDirection: "column" }}>                  <Radio className="custom-radio-btn" value={1}>All Users</Radio>                  <Radio className="custom-radio-btn" value={2} style={{ paddingTop: "6px" }}>Select Users</Radio>                </Radio.Group>              </Form.Item>            </Col>            <Col xs={24} md={24}>              <div
              style={{
                display: "flex",
                justifyContent: "end",
                marginBottom: "0px",
      ...
      {selectedUsers && <Tabs defaultActiveKey="1" items={tabItems} onChange={onTabChange} />}
    </Modal>);
}
