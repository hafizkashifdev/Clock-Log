    ...
    return (
        <Row className="showTime">
            <Col md={12} sm={24} className="addTimeButton">
                <button style={{}} onClick={() => { showModal(); props.setSelectedRecord({}); form.resetFields(); setView(false) }} className="d-flex border-radius-4">
                    <img src={addLocationIcon} width="19px" height="21.7px" alt="icon" />
                    <p className="fs-16 fw-500 m-0">Add Location</p>
                </button>
            </Col>
            <Modal
                open={isOpen}
